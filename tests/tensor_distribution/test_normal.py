...
class TestTensorNormalInitialization:
    def test_init_no_loc_raises_error(self):
        ...
        with pytest.raises(
            ...
            TensorNormal(loc=None, scale=torch.tensor(1.0)) # type: ignore

    def test_init_no_scale_raises_error(self):
        """A RuntimeError should be raised when scale is not provided."""
        with pytest.raises(
            RuntimeError, match="'scale' must be provided."
        ):
            TensorNormal(loc=torch.tensor(0.0), scale=None) # type: ignore
