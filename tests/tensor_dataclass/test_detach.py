...
class TestTensorDataClassDetach:
    ...
    def test_detach_with_nested_dataclasses(self):
        ...
        assert torch.equal(
            detached_model.config.inner_weights, model.config.inner_weights
        )
    ...
    def test_detach_works_with_torch_compile(self):
        ...
        assert (
            network.encoder_weights.data_ptr()
            == compiled_result.encoder_weights.data_ptr()
        )
        assert (
            network.decoder_weights.data_ptr()
            == compiled_result.decoder_weights.data_ptr()
        )
