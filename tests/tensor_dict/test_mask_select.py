...
def simple_td():
    ...
    data = {"observations": torch.randn(4, 10), "actions": torch.randint(0, 5, (4, 1))}
...
def test_mask_select_1d(simple_td):
    ...
    assert torch.equal(
        selected_td.data["observations"], simple_td.data["observations"][mask]
    )
...
def test_mask_select_2d(td_with_2d_batch):
    ...
    assert torch.equal(
        selected_td.data["pixels"], td_with_2d_batch.data["pixels"][mask]
    )
    assert torch.equal(
        selected_td.data["rewards"], td_with_2d_batch.data["rewards"][mask]
    )
